...
## 10. SBESC 2026 Article

The article `conference_101719.tex` (located in `docs/SBESC_2026_Framework_BT_Dispositivos_Restrições_Recursos/`) has been fully drafted with the latest case study results. The structure now properly includes the abstract, synthesized introduction and framework explanation, the case study methodology and evaluation (incorporating Axes A-E), and a conclusion. The document compiles successfully using `latexmk` and is constrained within the 6-page limit.
