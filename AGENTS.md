...
- **Axis C — Flash/RAM footprint**: What is the static memory footprint for each implementation?
  Measured by cross-compiling for `nucleo_f091rc` and reading ELF section sizes.
...
Cross-compiles the 4 variants (BT/FSM x base/tamper) for `nucleo_f091rc` (Cortex-M target)
...
# The `report.py` script automatically reads the template `docs/evaluation_report_pt_template.md`,
# fills in the CSV values, and writes the output to `docs/evaluation_report_pt_generated.md`.
# You may rename this file to reflect the target board (e.g. `docs/evaluation_report_pt_nucleo.md`).
```
...
7. **Report Generation Pipeline**: Whenever asked to "generate the report", an agent MUST follow the pipeline described in Section 7. It must execute the evaluation scripts (ending with `report.py`) which will automatically generate `docs/evaluation_report_pt_generated.md` from the template. The agent must never hallucinate numbers; they must always come from this automated pipeline.
