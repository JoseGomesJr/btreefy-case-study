...
\author{\IEEEauthorblockN{Victor R. S. de Oliveira, Rodrigo Peixoto, ... , Bruno Nogueira}
\IEEEauthorblockA{\textit{Instituto de Computação} \\
\textit{Universidade Federal de Alagoas}\\
Maceió, Brasil \\
\{vrso, rodrigopex, \}@ic.ufal.br}
\and
...
\red{
Alfredo:
Um outro formalismo que vem ganhando destaque é o \textit{Behavior Tree} (BT), que se tornou bastante popular em aplicações de robótica e Inteligência Artificial (IA) nos últimos anos...
}
\textit{Behavior Trees} (BTs) são um formalismo de controle que se tornou bastante popular em aplicações de robótica e Inteligência Artificial (IA) nos últimos anos. Originalmente, \textit{Behavior Trees} foram projetadas para superar limitações encontradas no desenvolvimento de inteligência artificial para personagens de jogos digitais utilizando Máquinas de Estados Finitos, por permitirem maior modularidade e reatividade. Ao longo dos anos, tem-se observado uma crescente utilização de BTs em diversas áreas, como a robótica, sistemas autônomos, IA e controle industrial. Apesar do uso extensivo em diversas áreas, a aplicação de BTs em sistemas embarcados tem sido pouco investigada, muito embora a sua natureza fundamentalmente baseada em eventos (através da propagação de \textit{ticks}) alinhe-se de maneira natural à dinâmica de aplicações de sistemas embarcados, proporcionando abstrações modulares para lidar com interrupções e mudanças operacionais.

\red{Alfredo: Acho que o texto acima pode ser melhor sintetizado. Exemplo: já falou que está sendo aplicado em robótica e IA, então dizer que outras áreas como sistemas autônomos e controle industrial também estão utilizando.}
...
\red{Alfredo: Acho que está faltando falar que o modo de execução sempre é do filho mais à esquerda para a direita. Motivando a LCRS.
}

\blue{
Em cada nó você comenta que é da esquerda para direita, mas não tão forte quanto dizer que em qualquer nó de controle isso ocorre. Além disso, é necessário enfatizar que é uma forma de prioridade.
}
...
\blue{Enfatizar que esse processo é naturalmente recursivo. Por isso a parte da mudança para modo iterativo em embarcados.}
...
\green{Em uma abordagem recursiva, o processador precisa empilhar \textbf{em cada chamada} o contexto de execução atual...}

% \begin{itemize} % Lista de características do BTreeFy - Início

\paragraph{\textbf{Utilização da representação LCRS}} para facilitar a definição da estrutura de dados de um nó da \textit{Behavior Tree} foi utilizada a representação \textit{left-child right-sibling} para construção da estrutura da árvore, \red{sendo essa representação um ponto-chave para utilização de BTs em Sistemas Embarcados com Recursos Restritos}.

\red{
Alfredo - ideia:
Uma árvore pode ser representada de diferentes maneiras, variando desde estruturas baseadas em ponteiros até representações que armazenam os nós em um array. Em ambas as abordagens, o objetivo é preservar as relações de parentesco entre os nós, diferindo apenas na forma como essas relações são codificadas. Enquanto as representações baseadas em ponteiros estabelecem ligações diretas entre os nós por meio de referências de memória, as representações baseadas em arrays utilizam posições ou índices para descrever essas relações. Cada uma dessas estratégias apresenta vantagens e desvantagens em termos de consumo de memória, localidade de referência, facilidade de implementação e eficiência das operações sobre a árvore. Dessa forma, a escolha da representação deve considerar as características da aplicação e os requisitos de desempenho e armazenamento.
}

\red{
Dado esse contexto, a primeira decisão de \green{arquitetura -- acho que pode ser escolhido  uma palavra melhor --} consiste em armazenar todos os nós da árvore em um único array, representando as relações entre eles por meio de índices em vez de ponteiros.
Essa abordagem reduz o consumo de memória, melhora a localidade de referência e favorece o desempenho das operações de acesso devido ao melhor aproveitamento da hierarquia de cache.
}

\red{
O segundo ponto é escolher como representar a árvore no array.
Uma árvore armazenada em um array pode ser representada de diferentes formas, variando conforme a estratégia adotada para codificar as relações entre os nós. As abordagens mais comuns incluem representações baseadas no índice do pai (parent array), no primeiro filho e próximo irmão (Left-Child Right-Sibling – LCRS), em vetores de filhos, em estruturas compactas como Compressed Sparse Row (CSR) e em representações implícitas, como as utilizadas em árvores binárias completas. Cada uma dessas alternativas apresenta características distintas em termos de consumo de memória, localidade de referência, complexidade das operações de navegação e facilidade de atualização da estrutura. Dessa forma, a escolha da representação deve considerar as propriedades da árvore e os requisitos da aplicação, buscando um equilíbrio entre eficiência de armazenamento e desempenho computacional.
}

\green{Fazer uma comparação entre as formas, ... indicar a transformação Knuth k-ary para binária (LCRS)}

Nessa representação, um nó precisa apenas referenciar dois nós: seu nó filho mais à esquerda e o seu irmão, que é o nó imediatamente à sua direita, conforme pode ser observado na figura [ADICIONAR FIGURA].
...
\red{Acho que aqui pode indicar que dado a forma como representamos a árvore é fazer fazer a geração de código de forma otimizada, que isso não é possível -- eu acho --  com máquina de estados.}

Dito isto, novamente pergunto, preciso tomar alguma ação no sistema para matricular novamente?% \end{itemize} % Lista de características do BTreeFy - Fim
...
\red{Dois pontos a ser melhorado 32 bits para índice em execução e tamanho, exagerado}
...
Nesta seção será apresentado um estudo de caso demonstrando a implementação da aplicação de rastreamento apresentada na figura \ref{fig:bt_example}. Para o desenvolvimento da aplicação foram utilizados o \textit{framework} proposto, \textbf{BTreeFy}, e o sistema operacional de tempo real \textit{Zephyr}. A aplicação foi executada em uma placa NUCLEO-FO91RC \cite{noauthor_nucleo-f091rc_nodate} com ARM Cortex-M0, 256KB de Flash e 32KB de RAM.
